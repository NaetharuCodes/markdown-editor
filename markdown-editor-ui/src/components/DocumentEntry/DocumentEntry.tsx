...
const DocumentEntry = ({ date, title, onClick }: DocumentEntryProps) => {
  return (
    <button className={styles.button} onClick={onClick}>
      ...
      <div className={styles.textContainer}>
        <div className={`${styles.date} app-text`}>{date}</div>
        <div className={`${styles.title} app-heading-med`}>{title}</div>
      </div>
    </button>
  );
};
